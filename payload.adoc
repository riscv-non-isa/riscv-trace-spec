...
manner in which packets are encapsulated for transport is not mandated, but the https://docs.riscv.org/reference/debug-trace-ras/trace-encap/_attachments/e-trace-encap.pdf[Unformatted Trace & Diagnostic Data Packet Encapsulation for RISC-V Specification] is recommended.  Chapter 3 of that document specifically addresses the encapsulation requirements for E-Trace.
...
[align="center",float="center",cols="<15%,<15%,<~",options="header"]
|===
...
[align="center",float="center",cols="<15%,<6%,<~",options="header"]
|===
| *Field name*| *Bits*| *Description*
|*format*| 2| 11 (sync): synchronisation
...
[NOTE]
====
The above packet is deprecated and has been superseded by the equivalent packet defined in the Standard Support Packet extension (see <<sec:ext-support>>).
====
...
.Packet format 2
[align="center",float="center",cols="<12%,<24%,<~",options="header"]
|===
| *Field name*| *Bits*| *Description*
...
|*irdepth*| _return_stack_size_p_ +
           + (_return_stack_size_p_ > 0 ? 1 : 0) +
           + _call_counter_size_p_| If the value of *irreport* is different from
...
[NOTE]
====
The above packet is deprecated and has been superseded by the equivalent packet defined in the Implicit Return extension (see <<sec:ext-impret>>).  This replaces *irdepth* with *irets*.
====
...
[NOTE]
====
*irdepth* has been superseded by *irets* defined in the Implicit Return extension (see <<sec:ext-impret>>).
====
...
privilege change or resync because it is necessary to report the current
address stack depth or nested call count.
|*irdepth*| _return_stack_size_p_ + (_return_stack_size_p_ > 0 ? 1 : 0)
+ _call_counter_size_p_| If the value of *irreport* is different from
*updiscon*, this field indicates the number of entries on the return
address stack (i.e. the entry number of the return that failed) or
nested call count. If *irreport* is the same value as *updiscon*, all
...
[align="center",float="center",cols="<14%,<24%,<~",options="header"]
|===
| *Field name*| *Bits*| *Description*
