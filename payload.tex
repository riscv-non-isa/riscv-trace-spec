...
\begin{table}[htp]
  ...
  \begin{tabulary}{\textwidth}{|l|p{35mm}|p{90mm}|}
    ...
    \textbf{tvalepc} & \textit {iaddress\_width\_p} & 
           Exception address if \textbf{ecause} is 2 or 22 and \textbf{interrupt} is 0 (illegal instruction or virtual instruction exception), or trap value otherwise.\\
    \hline
  \end{tabulary}
\end{table}
  ...
  \begin{tabulary}{\textwidth}{|l|p{35mm}|p{90mm}|}
    \hline
     ...
     \textbf{qual\_status} & 2 & Indicates qualification status\newline
       ...
       11 : (\textcolor{green}{ended\_ntr}): Qualification ended, preceding \textbf{te\_inst} would have been sent anyway due to an updiscon, even if it wasn't the last qualified instruction)\\
     \hline
...
\textbf{ended\_rep} and \textcolor{green}{\textbf{ended\_ntr}}.  This relates to exactly the same ambiguous case described in detail in 
...
\begin{itemize}
  ...
  \item \textcolor{green}{\textbf{ended\_ntr}} indicates that the preceding packet would have been issued anyway because of an uninferable
