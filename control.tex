...
\begin{itemize}
  ...
  \item \textcolor{cyan}{\textbf{RW} column heading indicates whether bit is read-only (R), or read-write (RW).  
...
\begin{table}[htp]
  ...
  \begin{tabulary}{\textwidth}{|l|l|l|l|l|p{90mm}|}
    ...
    \hline
    \textbf{\textcolor{cyan}{teEnable}} & \textcolor{cyan}{1} & \textcolor{cyan}{M} & \textcolor{cyan}{RW} & \textcolor{cyan}{0} & 
      \textcolor{cyan}{Master trace enable.  Trace can be enabled via \textbf{iEnable}
      or \textbf{dEnable} when 1. Setting to 0 flushes any queued trace data to the designated sink.}\\
    \hline
    \textbf{\textcolor{cyan}{iTracing}} & 1 & M & RW & 0 & Instruction trace enable.  \textcolor{cyan}{When 1, trace will be generated,
       subject to any optional filtering.  May be written by software, or via triggers if \textbf{iTrigEnable} is 1.}  \\
    \hline
    \textbf{\textcolor{cyan}{dTracing}} & 1 & MD & RW & 0 & Data trace enable.  \textcolor{cyan}{When 1, trace will be generated,
       subject to any optional filtering.  May be written by software, or via triggers if \textbf{dTrigEnable} is 1.}  \\
    \hline
    \textbf{iTrigEnable} & 1 & O & RW & 0 & When 1, allows \textcolor{cyan}{iTracing} to be set or cleared by \textit{trace-on} and \textit{trace-off} 
      Debug module triggers respectively (see \ref{sec:trigger}).\\
    \hline
    \textbf{dTrigEnable} & 1 & OD & RW & 0 & When 1, allows \textcolor{cyan}{dTracing} to be set or cleared by \textit{trace-on} and \textit{trace-off} 
      Debug module triggers respectively (see \ref{sec:trigger}).\\
    \hline
...
\begin{table}[htp]
  ...
  \begin{tabulary}{\textwidth}{|l|l|l|l|l|p{80mm}|}
    ...
    \textbf{matchChoicePrivilege} & 8 & O & RW & SD & When \textbf{matchPrivilege} is set, match all privilege levels for which the corresponding bit is set.
       For example, if bit \textit{N} is 1, then match if the privilege level is \textit{N}.\\
    \hline
