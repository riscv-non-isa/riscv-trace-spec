...
As outlined in <<sec:DataInterfaceRequirements>>, two different signaling protocols between the RISC-V hart and the encoder are supported: _unified_ and _split_. Accordingly, both unified and split trace packets are defined.  Where the _split_ signalling is supported, data accesses with a load data component normally result in two packets, one each for the request and data phases.  However, if address or data are omitted as described above, such that all the required information is from a single phase, then the appropriate unified packet format will be used.  For example, if the data value is omitted then the _Unified load or store with address only_ packet format will be used even if _split_ signalling is used.  Similarly, if the address is omitted then _Unified load or store with data only_ is used.
...
[NOTE]
====
The packets described in this section do not support data trace for harts that support the Zcmp, Vector or Zacas extensions, and are deprecated in favour of the packets defined in the MMA/CAS extension (see <<sec:ext-mmacas>>)
====
...
[NOTE]
====
The above packet is deprecated and has been superseded by the equivalent packet defined in the MMA/CAS extension (see <<sec:ext-mmacas>>)
====

The following packet format is used when the encoder is configured to omit data values from data trace packets (trTeDataControl.trTeDataNoValue is 1), even with _split_ siganalling.
...
The following packet format is used when the encoder is configured to omit address values from data trace packets (trTeDataControl.trTeDataNoAddr is 1), even with _split_ siganalling.
...
.Packet format for Split load - data phase
[%autowidth,align="center",float="center",cols="<,<,<",options="header"]
|===
| *Field name* | *Bits* | *Description*
|*format* | 3 | Transaction type +
...
need for performing subtraction of large values.  Encoded values are determined on a per access size basis.  For example, when using XOR compression, the  address of this data access, XOR the address of the previous data access of the same size.
...
|*size* | max(1, clog2(clog2( _data_width_p_/8 + 1))) | Transfer size is
2^*size*^ bytes
|*diff* | 2 | 00: Full address and data/operand (sync) +
...
The following packet format is used when the encoder is configured to omit address values from data trace packets (trTeDataControl.trTeDataNoAddr is 1), and supports _unified_ signalling.
...
.Packet format for Split atomic - request phase
[%autowidth,align="center",float="center",cols="<,<,<",options="header"]
|===
| *Field name* | *Bits* | *Description*
|*format* | 3 | Transaction type +
...
.Packet format for Split atomic without address - request phase
[%autowidth,align="center",float="center",cols="<,<,<",options="header"]
|===
| *Field name* | *Bits* | *Description*
|*format* | 3 | Transaction type +
...
The above packet is deprecated and has been superseded by the equivalent packet defined in the MMA/CAS extension (see <<sec:ext-mmacas>>)
====
.Packet format for Split atomic load - data phase
[align="center",float="center",cols="<10%,<17%,<~",options="header"]
|===
| *Field name* | *Bits* | *Description*
|*format* | 3 | Transaction type +
