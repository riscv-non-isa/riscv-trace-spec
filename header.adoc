...
Authors: Gajinder Panesar and Iain Robertson
include:../docs-resources/global-config.adoc[]
:description: Efficient Trace for RISC-V
:company: RISC-V.org
:revdate: TBD, 2026
:revnumber: 2.0
:revremark: This document is in the Ratified state.
...
[WARNING]
.This document is in the link:http://riscv.org/spec-state[Ratified  state] except where explicitly stated in some  extension chapters.
====
...
* Added <<sec:ext-support>>, which describes the Standard Support Packet extension to Version 2.0.
* Added <<sec:ext-impret>>, which describes the Implicit Return extension to Version 2.0.
* Added <<sec:ext-mmacas>>, which describes the Data Trace of Multiple-Memory Access and Compare and Swap extension to Version 2.0.
...
* Corrections to <<encoderControl>>: *trTeInstMode* hardcoded to 6 (not 7 as previously stated), *trTeContext* and *trTeInstImplicitReturnMode* changed to optional rather than not-applicable.
* Fixed error in *branch_map* descriptions in tables in <<sec:format1>> introduced during earlier translation from LaTeX to asciidoc: values for branch taken or not taken were missing.
* Removed inappropriate and confusing references to "time changes" from <<sec:InstructionInterfaceRequirements>>.
* Added informative recommendation regarding the use of partial timestamps in packet encapsulation to <<ch:timestamping>>.
* Remove historical "Future directions" chapter.
...
* Updated <<packets>> and <<fragments>> to reference the https://docs.riscv.org/reference/debug-trace-ras/trace-encap/_attachments/e-trace-encap.pdf[Unformatted Trace & Diagnostic Data Packet Encapsulation for RISC-V Specification];
...
* Control field definitions removed from section 2, which now references the xref:https://docs.riscv.org/reference/debug-trace-ras/trace-control-interface/_attachments/RISC-V-Trace-Control-Interface.pdf[RISC-V Trace Control Interface Specification]
