...
global bool   inferred_address = FALSE    # Flag to indicate that reported address from format 0/1/2 was
                                          # not following an uninferable jump (and is therefore inferred)
global bool   start_of_trace = TRUE       # Flag indicating 1st trace packet still to be processed
global        address                     # Reconstructed address from te_inst packets
global        privilege                   # Privilege from te_inst packets
...
global struct previous_te_inst            # Copy of previous te_inst packet
...
function process_te_inst (te_inst)
  ...
  if (te_inst.format == 3)
    ...
    if (is_branch(get_instr(address))) # 1 unprocessed branch if this instruction is a branch
      ...
      branches++
    if (te_inst.subformat == 0 and !start_of_trace and
        !(previous_te_inst.format == 3 and previous_te_inst.subformat == 1 and !previous_te_inst.thaddr))
  ...
  while (TRUE)
    if (inferred_address) # iterate again from previously reported address to find second occurrence
      ...
      !unprocessed_branches(pc) and !unproc_ir)
        ...
        return
      if (!stop_at_last_branch and pc == address and !is_uninferable_discon(get_instr(last_pc) and
        ...
        # uninferable jump target.  Stop here for now, though flag indicates this may not be final retired
        # instruction
...
function next_pc (address, te_inst)
  ...
  if (is_inferrable_jump(instr))
    ...
    if (stop_at_last_branch and !is_branch(address))
      ...
      pc = address
      if (!exception)
        if (!flag_irreport(te_inst) or address != popped_return_target)
          # Update cache unless address is correctly predicted but reported for some other reason
          cache_update(pc)
      stop_here = TRUE
...
function is_return (instr)
  ...
  if ((instr.opcode == JALR and (instr.rs1 == 1 or instr.rs1 == 5) and instr.rd != 1 and instr.rd != 5) or
      (instr.opcode == C.JR and (instr.rs1 == 1 or instr.rs1 == 5)) or
      (instr.opcode == CM.POPRET))
    return TRUE
...
function exception_address (te_inst)
  ...
  if (start_of_trace or is_uninferable_discon(instr))
  ...
  if ((instr.opcode == ECALL) or (instr.opcode == EBREAK) or (instr.opcode == C.EBREAK))
    return pc
  if (!te_inst.thaddr) # pne_valid is false.  te_inst address is EPC of current trap, not next trap
    return (te_inst.address << discovery_response.iaddress_lsb)
