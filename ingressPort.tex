...
needed between the core and the encoder, and should have a minimal impact on trace efficiency as it is anticipated that
...
\begin{table}[htp]
    \centering
    \caption{Core-Encoder signals - sequentially inferable jumps}
...
\textbf{sijump} is optional and may be omitted if the core does not implement the logic to detect
...
\begin{table}[h]
    ...
    \begin{tabulary}{\textwidth}{|l|p{25mm}|p{80mm}|}
        ...
        \hline
        \textit{sijump\_p} & & \textbf{sijump} is used to identify sequentially inferable jumps\\
        \hline
...
\subsection {Discovery of parameter values} \label{sec:disco}
...
\begin{itemize}
    ...
    \item \textit{cache\_size}. Number of entries in the jump target cache is 2\textsuperscript{\textbf{cache\_size}}.  No cache if 0.
    ...
    \item \textit{f0s\_width}. Width of the \textbf{subformat} field in format 0 \textit{te\_inst} packets (see section~\ref{sec:f0s}).
    ...
    \item \textit{sijump}. Sequentially inferable jumps supported when 1.
