...
\begin{itemize}
  ...
  \item Whether there has been an exception or interrupt, and if so the cause 
        (from the \textbf{\textit{ucause/scause/\textcolor{cyan}{vscause/}mcause}} CSR)
        and trap value (from the \textbf{\textit{utval/stval/\textcolor{cyan}{vstval/}mtval}} CSR).
...
\begin{itemize}
  ...
  \item MR: Mandatory, may be replicated. For harts that can retire a maximum of N \textcolor{cyan}{"special"} instructions per clock
    cycle, the interface must include N instances of this signal.
  \item OR: Optional, may be replicated. For harts that can retire a maximum of N \textcolor{cyan}{"special"} per clock
...
\end{itemize}
\textcolor{cyan}{"Special" instructions are those that require \textbf{itype} to be non-zero.}
...
\begin{table}[htp]
    ...
    \begin{tabulary}{\textwidth}{|l|l|p{80mm}|}
        ...
        \hline
        \textbf{cause}[\textit{ecause\_width\_p}-1:0] & M & Exception or interrupt cause (\textbf{\textit{ucause/scause/ \textcolor{cyan}{vscause/}mcause}}).
        ...
        written to the \textbf{utval/stval/\textcolor{cyan}{vstval/}mtval} CSR. Future optional extensions may define \textbf{tval} 
        ...
        Ignored for \textbf{itype} codes other than \textcolor{cyan}{6,} 8, 10, 12 or 14.\\
        \hline
    \end{tabulary}
\end{table}
...
\begin{table}[htp]
    ...
    \begin{tabulary}{\textwidth}{|l|p{140mm}|}
      ...
      \textbf{Value} & \textbf{Description} \\
      \hline
        \textcolor{cyan}{0} & \textcolor{cyan}{U}\\
        \textcolor{cyan}{1} & \textcolor{cyan}{S/HS}\\
        \textcolor{cyan}{2} & \textcolor{cyan}{reserved}\\
        \textcolor{cyan}{3} & \textcolor{cyan}{M}\\
        \textcolor{cyan}{4} & \textcolor{cyan}{D (debug mode)}\\
        \textcolor{cyan}{5} & \textcolor{cyan}{VU}\\
        \textcolor{cyan}{6} & \textcolor{cyan}{VS}\\
        \textcolor{cyan}{7} & \textcolor{cyan}{reserved}\\
      \hline
    \end{tabulary}
\end{table}
...
For harts that can retire a maximum of N \textcolor{cyan}{non-zero \textbf{itype} values} per clock
cycle, the signal groups MR, OR and either BR or SR must be replicated N times.  
\textcolor{cyan}{Typically N is determined by the maximum number of branches that can be retired per clock cycle.}
...
than one privilege \textcolor{cyan}{change}, context \textcolor{cyan}{change}, 
...
be zero for higher numbered groups). If fewer than N \textcolor{cyan}{groups are required} 
...
\textcolor{cyan}{starts and stops.  It is recommended that tracing starts from the oldest  
...
\textcolor{cyan}{(subject to any optional filtering)}.  
