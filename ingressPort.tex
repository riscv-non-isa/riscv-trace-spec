...
For example, if instructions are retired in blocks, and assuming the hart can perform one load or store per cycle,
the following retirement sequences could be reported when pushing and popping 4 registers from the stack:

\begin{enumerate}
  \item Last instruction in block is \textit{push}, \textbf{itype} 0 (data trace interface reports 1st store);
  \item Single \textit{push}, \textbf{itype} 0 (data trace interface reports 2nd store);
  \item Single \textit{push}, \textbf{itype} 0 (data trace interface reports 3rd store);
  \item First instruction in block is \textit{push}, \textbf{itype} dependent on last instruction in block (data trace interface reports 4th store);
\end{enumerate}

\begin{enumerate}
  \item Last instruction in block is \textit{popret}, \textbf{itype} 0 (data trace interface reports 1st load);
  \item Single \textit{popret}, \textbf{itype} 0 (data trace interface reports 2nd load);
  \item Single \textit{popret}, \textbf{itype} 0 (data trace interface reports 3rd load);
  \item Single \textit{popret}, \textbf{itype} 13 (data trace interface reports 4th load);
\end{enumerate}
...
\textbf{Note:} This is required for data trace only.  If data trace is not implemented, the push or pop
