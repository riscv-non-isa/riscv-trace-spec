  ...
  while (TRUE)
    if (inferred_address) # iterate again from previously reported address to
      ...
      if (te_inst.format != 3 and pc == address and 
        (te_inst.notify != te_inst.address[MSB]) and !stop_at_last_branch and
        ...
        return
      if (te_inst.format != 3 and pc == address and
        (te_inst.updiscon == te_inst.notify) and !stop_at_last_branch and
        (branches == (is_branch(get_instr(pc)) ? 1 : 0)))
...
function is_implicit_return (instr)
  ...
  if ((instr.opcode == JALR and instr.rs1 == 1 and instr.rd == 0) or
    ...
    if (implicit_return_fail(te_inst) and te_inst.ircount == call_counter)
      return FALSE 
...
# Check for implicit_return failure #
function implicit_return_fail(te_inst)

  local previous_bit = te_inst.updiscon
  if (te_inst.format == 0 and te_inst.subformat == 0)
    previous_bit = te_inst.branches == 0 ? te_inst.branches[4]    :
                   te_inst.branches <  2 ? te_inst.branch_map[0]  : 
                   te_inst.branches <  4 ? te_inst.branch_map[2]  : 
                   te_inst.branches <  8 ? te_inst.branch_map[7]  : 
                   te_inst.branches < 16 ? te_inst.branch_map[15] : 
                                           te_inst.branch_map[31]
                   
  return te_inst.irfail != previous_bit
