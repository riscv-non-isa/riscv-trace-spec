...
\textcolor{cyan}{Note: in the following tables, "clog2" is an abbreviation for "ceiling of log2".}
...
\begin{table}[htp]
  ...
  \begin{tabulary}{\textwidth}{|l|p{38mm}|p{92mm}|}
    ...
    \textbf{data} & 8 * (\textbf{data\_len} + 1) & 
                Data\\
    \hline
    \textbf{address} &  \textcolor{cyan}{\textit{daddress\_width\_p}} & Byte address if format is unaligned, otherwise shift left by \textbf{size} to recover byte address \\
    \hline
  \end{tabulary}
\end{table}
...
\subsection{\textcolor{cyan}{data\_len field}} \label{sec:loadstore-datalen}

\textcolor{cyan}{However the data is compressed, upper bytes that are all the same value do not need to be 
...
\begin{table}[htp]
  ...
  \begin{tabulary}{\textwidth}{|l|p{38mm}|p{92mm}|}
    ...
    \textbf{data} & 8 * (\textbf{data\_len} + 1) & 
                Data\\
    \hline
    \textbf{address} &  \textcolor{cyan}{\textit{daddress\_width\_p}} & Address, aligned and encoded as per size \\
    \hline
  \end{tabulary}
\end{table}
...
\subsection{\textcolor{cyan}{operand field}} \label{sec:atomic-operand}

\textcolor{cyan}{The operand value for the atomic operation.  Uncompressed, although upper bytes 
...
\subsection{\textcolor{cyan}{data\_len and op\_len fields}} \label{sec:atomic-datalen}

\textcolor{cyan}{Width of \textbf{data} and \textbf{operand} fields respectively.
...
\begin{table}[htp]
  ...
  \begin{tabulary}{\textwidth}{|l|p{38mm}|p{92mm}|}
    ...
    \hline
    \textbf{op\_len} & 2 or 3  & Number of bytes of \textcolor{cyan}{operand} is \textbf{op\_len} + 1\\    
    \hline
    \textbf{operand}	& 8 * (\textbf{op\_len} + 1) & Operand.  Value from \textcolor{cyan}{rs1} before operator applied\\
    \hline
...
\subsection{\textcolor{cyan}{operand field}} \label{sec:csr-operand}

\textcolor{cyan}{See section~\ref{sec:atomic-operand}.}

\subsection{data\_len \textcolor{cyan}{and op\_len} fields} \label{sec:csr-datalen}

2 bits wide if hart has 32-bit CSRs, 3 bits if 64-bit.  \textcolor{cyan}{Width of 
