[[sec:ext-support]]
== Standard Support Packet v0.9 extension to Efficient Trace for RISC-V v2.0

[WARNING]
.This chapter is in the link:http://riscv.org/spec-state[Frozen state]
====
Changes are unlikely at this point of the specification. Visit http://riscv.org/spec-state for further details.
====
...
Additionally, the fully orthogonal Implicit Return and Multi-Memory Access and Compare and Swap extensions introduce revised *te_inst* and *te_data* packet formats, but the monotonically increasing protocol version does not provide a mechansim to indicate independent presence or absence of these extentions (see trTeImpl.trTeProtocolMajor and trTeImpl.trTeProtocolMinor in https://docs.riscv.org/reference/debug-trace-ras/trace-control-interface/_attachments/RISC-V-Trace-Control-Interface.pdf[RISC-V Trace Control Interface Specification]).  This can resolved by indicating the presence of thes extensions directly in the support packet.
...
[align="center",float="center",cols="<20%,<6%,<~",options="header"]
|===
...
|*time_width*| 3 | The width of the time field in sync-start and sync-trap packets (_time_width_p_) is 16 * *time_width* bits.
...
[[sec:ext-impret]]
== Implicit Return v0.9 extension to Efficient Trace for RISC-V v2.0
...
[align="center",float="center",cols="<14%,<20%,<~",options="header"]
|===
| *Field name*| *Bits*| *Description*
...
[align="center",float="center",cols="<14%,<5%,<~",options="header"]
|===
| *Field name*| *Bits*| *Description*
|*format*| 2| 01 (diff-delta): includes branch information and may
include differential address
...
[align="center",float="center",cols="<15%,<20%,<~",options="header"]
|===
| *Field name*| *Bits*| *Description*
...
[[sec:ext-mmacas]]
== Data Trace of Multiple-Memory Access and Compare and Swap v0.9 extension to Efficient Trace for RISC-V v2.0
...
|===
| *Field name* | *Bits* | *Description*
|*format* | clog2(_dtype_width_p_) | Transaction class: +
00: Load or store +
...
The following packet is used for the data phase of a split load and split atomic.
...
2^*size*^ bytes
|*lrid* | _lrid_width_p_ | Load request ID
|*diff* | 2 | 00: Full address and operand (sync) +
01: Differential address, XOR-compressed operand +
10: Differential address, full operand +
11: Differential address, differential operand
|*op_len* | *size* | Number of bytes of operand is *op_len* + 1
...
2^*size*^ bytes
|*lrid* | _lrid_width_p_ | Load request ID
|*diff* | 2 | 00: Full operand (sync) +
01: XOR-compressed operand (XOR if 2 bits)+
10: reserved +
11: Differential operand
