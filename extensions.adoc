...
The implicit return mode information in the format 0/1/2 packets defined in the Efficient Trace for RISC-V 2.0 specification does not produce correct trace decoding in some cases.  This extension corrects this by replacing the *irdepth* field with *irets*.

==== Background
As described in section <<sec:implicit-return>>, the target address of function returns is not reported unless the function returned to an unxpected address, or where the instruction at the return target address is the last before an interrupt or exception, or the final instruction traced.  When this occurs, in most cases the decoder can correctly determine which return target has been reported, but there are some circumstances where this cannot be determined precisely without additional information from the encoder.

One such circumstance occurs when decoding recursive functions.  Each recursively called function will have the same return target address, and including the depth of the return address stack allows the decoder to determine at which has been reported.  However, it is now known that providing the stack depth results in incorrect decoding in other cases unrelated to recursive functions.  Consider the following illustration:
...
Note that the stack depth is the same after the 1st function returns and after the 2nd function returns. This means that when the decoder reconstructs the execution path based on *irdepth*, it encounters the 1st return and because the stack depth in the _te_inst_ packet matches the current stack depth it stops decoding at this point, rather than carrying on to the second return.

Had there been any branches between the two functions or inside the 2nd function, decoding would have continued until all branches were processed (the 1st return would have been correctly treated as inferred), and decoding would have stopped at the right place: the return target of the 2nd function. Similarly if there were uninferrable calls/jumps.
...
The following packet definitions replace those detailed in section <<sec:packets>>.  All formats and fields are the same, except *irdepth* is replaced by *irets* throughout.
...
These bits are encoded so that most of the time they will take the same value as the *updiscon* field, and will therefore compress away, having no impact on the encoding efficiency.  If implicit_return mode is enabled, the encoder keeps track of function calls and returns, and does not generate _te_inst_ packets for return target addresses that can be inferred by the decoder, as described in section <<sec:implicit-return>>.

When reporting an address via _te_inst_ when implicit_return mode is enabled, the packet will include details of the number of implicit returns (i.e. return targets that have not been reported explicitly) since the last branch or _te_inst_ packet under the following conditions:
