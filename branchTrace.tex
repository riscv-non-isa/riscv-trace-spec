...
\begin{itemize}
  ...
  \item Sequentially inferable jump mode:
    The target of an indirect jump can be infered by considering the combined effect of two instructions. 
...
can be considered inferable under some circumstances.  The core must identify jumps with 
...
the encoder and decoder synchronized, a cache with identical behavior will need to be modelled in the 
decoder software.  Even a small cache can provide significant improvement.

The cache shall comprise 2\textsuperscript{\textit{cache\_size\_p}} entries.  It will be one-way set 
associative, with each entry indexed by bits N:1 of the instruction address (or N+1:2 if compressed 
instructions aren't supported), and operate a simply least recently used eviction policy.  

Each uninferable jump target is first compared with the entries in the cache.  If it is found in the cache,
the index number is traced rather than the target address.  If it is not found in the cache, the 
least recently hit entry is discarded and replaced.
